...
class Clinica(models.Model):
    """Represents a corporate medical group or clinic.

    Groups multiple individual leads (doctors) under a single corporate entity.
    Provides aggregated tracking for high-volume customers.

    Attributes:
        nombre (CharField): Name of the clinic or medical group.
        telefono_master (CharField): Unique primary contact number for the clinic.
        ubicacion (ForeignKey): Geographical assignment linked to CatUbicacion.
...
class CoreLead(models.Model):
    """The central atomic unit of the CRM. Represents a single potential customer (Lead).

    Operates under a strict 3-Phase lifecycle enforced by validations and FSM transitions.
    Once a Lead advances past phase 1 (PROSPECTO), its core identity fields become mathematically immutable.

    Attributes:
        id (UUIDField): Internal primary key.
        owner (ForeignKey): The agent or user who owns the lead's conversion process.
        ubicacion (ForeignKey): The geographic region standardizing the lead for territory rules.
...
class LeadStaging(models.Model):
    """Intermediate staging table (Adapter Pattern) for mass ingested CSV/Excel data.

    Acts as a quarantine for defective or irregular leads. Sellers are forced to 
    manually resolve mapping conflicts, duplicates, or missing data before these 
    records are formally injected into the CoreLead table.

    Attributes:
        id (UUIDField): Obfuscated primary key.
        owner (ForeignKey): The user responsible for the upload/resolution.
        datos_crudos (JSONField): Raw row exact payload from the source file.
        datos_parseados (JSONField): Guessed mapping structures generated by the backend ingestor.
        motivo_conflicto (TextField): Reason for quarantine (e.g., 'Duplicate phone', 'Missing name').
        estatus (CharField): State of the quarantine (PENDIENTE, RESUELTO, DESCARTADO).
        origen (CharField): Indicates if the upload source is a mass historical DB or daily agent operation.
    """
    ESTATUS_CHOICES = [
...
class FiscalProfile(models.Model):
    """Invoicing and legal identity profile.

    Acts as a 1:1 satellite relative to CoreLead. Its creation is a strict requirement 
    for formalizing a lead into a CLIENTE (Phase 3). Contains immutable tax details.

    Attributes:
        lead (OneToOneField): Link to the CoreLead entity.
        rfc (CharField): Tax ID mapping.
        razon_social (CharField): Legal corporate name.
        regimen_fiscal (CharField): Official tax regime.
        calle, colonia, ciudad, estado, cp (CharField): Address vectors.
    """
...
class Notificacion(models.Model):
    """Stores system-generated or chronological alerts for specific users.

    Maintains the Observer pattern architecture by decoupling timed events from 
    immediate database commits. Generates UX badges on the user's dashboard.

    Attributes:
        usuario (ForeignKey): The user who must read the notification.
        lead (ForeignKey): The related CoreLead (makes the alert actionable/clickable).
        mensaje (CharField): Visual alert text.
        tipo (CharField): Internal categorization controlling icon/behavior.
        leida (BooleanField): Badge-clearing boolean indicating if it was reviewed.
        fecha_creacion (DateTimeField): Timestamp.
    """
    TIPO_CHOICES = [
